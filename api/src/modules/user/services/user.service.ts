...
export class UserService {
  ...
  public async create(
    ...
    const countPerformerUsername = data.username
    ...
    user.banks = [
      {
        ...
        routingNumber: user.bankRoutingNumber,
        branchName: user.bankBranchName,
        branchAddress: user.bankBranchAddress
      }
    ];
    delete user.bankSwiftCode;
    delete user.bankAccountType;
    delete user.bankAccountNumber;
    delete user.bankRoutingNumber;
    delete user.bankBranchName;
    delete user.bankBranchAddress;

    user.bodyType = {
      bodyHeight: user.bodyHeight,
      bodyWeight: user.bodyWeight,
      bodyRace: user.bodyRace,
      bodyBuild: user.bodyBuild
    };
    delete user.bodyHeight;
    delete user.bodyWeight;
    delete user.bodyRace;
    delete user.bodyBuild;
    ...
    user.verifiedEmail = true; // only for development purpose
  ...
  public async updateStripeCustomerId(
    userId: ObjectId,
  ...
  public async updateStripeCardIds(
    userId: ObjectId,
    cardId: string,
    isNew = true
  ) {
    isNew
      ? await this.userModel.updateOne(
        { _id: userId },
        { $push: { stripeCardIds: cardId } }
      )
      : await this.userModel.updateOne(
        { _id: userId },
        { $pull: { stripeCardIds: cardId } }
      );
  }
